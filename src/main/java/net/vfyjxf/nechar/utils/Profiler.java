                ...
                if (entry.getName()
                    ...
                    try (InputStream is = f.getInputStream(entry)) {
                        ...
                        if (size > Integer.MAX_VALUE) {
                            NotEnoughCharacters.logger.info(
                        ...
                        try (InputStream is = f.getInputStream(entry)) {
                            try {
                                ...
                                ret.mods = new ModContainer[] {
                                    gson.fromJson(new InputStreamReader(is), ModContainer.class) };
                            }
                        } catch (Exception e) {
                            NotEnoughCharacters.logger
                                .info("Fail to read mod info in jar file " + f.getName() + ", skip.");
                        }
                    }
            });
    ...
    private static void scanClass(InputStream is, Consumer<String> string, Consumer<String> regexp,
        ...
        try {
            ...
            if (classNode.name != null) {
                NotEnoughCharacters.logger
                    .info("File decoding of class " + classNode.name + " failed. Try to continue.");
            } else {
                ...
                if (node instanceof MethodInsnNode) {
                    ...
                    if (mNode.getOpcode() == Opcodes.INVOKEVIRTUAL && mNode.owner.equals("java/lang/String")
                        ...
                        && mNode.desc.equals("(Ljava/lang/String;)Z")) {
                            regexp.accept(
                                (classNode.name + ":" + methodNode.name + ":" + methodNode.desc).replace('/', '.'));
                            break;
                        } else
                        if (mNode.getOpcode() == Opcodes.INVOKEVIRTUAL && mNode.owner.equals("java/util/regex/Pattern")
                            ...
                            if (mNode.getOpcode() == Opcodes.INVOKESTATIC && mNode.owner.equals("kotlin/text/StringsKt")
                                && mNode.name.equals("contains")
