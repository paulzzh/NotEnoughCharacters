 ...
 * jecharacters/transform/Transformer.java)
 * 原文件协议为MIT
 */
...
public interface Transformer {
    ...
    static boolean transformInvoke(MethodNode methodNode, String owner, String name, String newOwner, String newName,
        ...
        while (iterator.hasNext()) {
            ...
            if (node instanceof MethodInsnNode
                && (node.getOpcode() == Opcodes.INVOKEVIRTUAL || node.getOpcode() == Opcodes.INVOKESPECIAL
            ...
            if (node instanceof InvokeDynamicInsnNode && node.getOpcode() == Opcodes.INVOKEDYNAMIC
                ...
                if (insnNode.bsmArgs[1] instanceof Handle) {
                    ...
                    if (h.getOwner()
                        ...
                        Object[] args = { Type.getType(arg1), new Handle(6, newOwner, newName, id),
                            Type.getType(arg2) };
    ...
    static void transformHook(MethodNode methodNode, String owner, String name, String id) {
        ...
        while (i.hasNext()) {
            ...
            if (node instanceof InsnNode && node.getOpcode() == Opcodes.RETURN) {
                methodNode.instructions
                    .insertBefore(node, new MethodInsnNode(Opcodes.INVOKESTATIC, owner, name, id, false));
            }
        }
    }
    ...
    abstract class Configurable extends Default {
        ...
        protected void transform(ClassNode c) {
            ...
            if (!ms.isEmpty()) c.methods.stream()
                .filter(m -> ms.contains(m.name))
